...
data_dir = '/home/saketh/Downloads/dr_data' ### path where the data is stored.
...
"""
def train_model(model, criterion, optimizer, scheduler, num_epochs=25, output_path='/home/bmi/Desktop/'):
    since = time.time()
...
model_ft= torchvision.models.densenet161(pretrained=True)  ### when false only the model architecture will be taken, if set to true then the network will be initilaized with weights learnt by training imagenet
...
model_ft = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler,num_epochs=50, output_path=output_name)
